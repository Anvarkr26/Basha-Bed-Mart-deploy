...
const UserMenu: React.FC = () => {
    ...
    return (
        <div className="relative" ref={menuRef}>
            <button onClick={() => setIsOpen(!isOpen)} className="flex items-center space-x-2 transition-transform duration-200 hover:scale-105">
            ...
            {isOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-50">
                    <Link to="/profile" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition-colors duration-150" onClick={() => setIsOpen(false)}>My Profile</Link>
                    <Link to="/orders" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition-colors duration-150" onClick={() => setIsOpen(false)}>My Orders</Link>
                    <button onClick={() => { logout(); setIsOpen(false); }} className="w-full text-left block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition-colors duration-150">Logout</button>
                </div>
            )}
        </div>
    );
...
const Header: React.FC = () => {
  ...
  const navLinkClasses = ({ isActive }: { isActive: boolean }) => 
    `py-2 text-gray-800 hover:text-primary transition-all duration-200 ${isActive ? 'text-primary font-semibold' : 'hover:-translate-y-px'}`;
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-4">
          ...
          <div className="flex items-center space-x-4">
            ...
            <Link to="/cart" className="text-gray-800 hover:text-primary relative transition-transform duration-200 ease-in-out hover:scale-110">
                <CartIcon />
            </Link>
            ...
            <div className="hidden md:flex items-center">
                {isLoggedIn ? (
                    ...
                    <Link to="/login" className="text-gray-800 hover:text-primary transition-transform duration-200 ease-in-out hover:scale-105">Login</Link>
                )}
            </div>
